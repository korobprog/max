...
let initialState = {
    ...
    totalUsersCount: 0,
    currentPage: 1,
    isFetching: false, // указатель загрузки контента
    followingInProgress: [2, 3] // disable button
};
...
const usersReducer = (state = initialState, action) => {
    switch (action.type) {
        case FOLLOW:
            ...
            return {
                ...state,
                users: state.users.map(u => { //user: [...state.users]
...
export const follow = (userId) => {
    return (dispatch) => {
        ...
        .then(response => {
            if (Number(response.data.resultCode) === 0) {
